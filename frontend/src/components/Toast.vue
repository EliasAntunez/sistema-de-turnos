<template>
  ...
  >
    <div
      ...
      :key="t.id"
      :class="[
        'pointer-events-auto flex items-start gap-3 rounded-xl px-4 py-3.5 shadow-xl bg-white border border-gray-100',
        accentClass(t.type)
      ]"
      style="width: 22rem"
      role="alert"
    >
      <!-- Icono semántico por tipo -->
      <span
        :class="['shrink-0 mt-0.5', iconColor(t.type)]"
      ...
      <div class="flex-1 min-w-0">
        <p :class="['text-sm font-semibold leading-snug', textColor(t.type)]">
          {{ t.message }}
        </p>
        <ul
          v-if="t.detalles && t.detalles.length"
          class="mt-1.5 pl-4 list-disc space-y-0.5 text-xs text-gray-500"
        >
...
function accentClass(type: ToastType): string {
  switch (type) {
    ...
    default:        return 'border-l-4 border-l-sky-500' // info + default
  }
}
