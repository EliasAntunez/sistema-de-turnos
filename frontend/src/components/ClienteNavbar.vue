<template>
  <nav class="relative z-40 bg-white border-b border-slate-200 shadow-sm">
      ...
      <div class="flex items-center justify-between h-14 sm:h-16">
        ...
        <div class="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
          ...
          <div class="flex items-center min-w-0">
            <div class="text-lg sm:text-xl font-bold text-indigo-600 sm:text-transparent sm:bg-clip-text sm:bg-gradient-to-r sm:from-indigo-600 sm:to-blue-600 truncate">
              {{ empresaNombre || 'Turnero' }}
            </div>
          </div>

          <!-- Separador solo desktop -->
          <div v-if="isAuthenticated && clientePerteneceAEmpresa" class="hidden sm:block h-5 w-px bg-slate-300"></div>
          ...
          <div v-if="isAuthenticated && clientePerteneceAEmpresa" class="hidden sm:flex items-center min-w-0">
            <div class="flex items-center gap-2 min-w-0">
              ...
              <div class="flex flex-col leading-tight min-w-0">
                <span class="text-xs text-slate-500 font-medium truncate">Bienvenido</span>
                <span class="text-sm font-semibold text-slate-900 truncate max-w-[120px] sm:max-w-[150px]" :title="clienteFullName">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
          ...
          <template v-if="isAuthenticated && clientePerteneceAEmpresa">
            ...
            <div class="md:hidden relative" ref="authMenuRef">
              <button
              ...
              >
                <div v-if="mobileMenuOpen" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-50">
                  <div class="px-4 py-3 border-b border-slate-100">
                    <div class="text-sm font-semibold text-slate-900 truncate">{{ clienteFullName || 'Usuario' }}</div>
                    <div class="text-xs text-slate-500">Bienvenido</div>
                  </div>
                  <button
                    @click="irAReservar"
                  ...
                  <button 
                    @click="cerrarSesion"
                    class="flex items-center gap-2 w-full px-4 py-3 text-sm text-red-600 hover:bg-slate-50 border-t border-slate-100"
                  >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="hidden md:flex items-center gap-2">
              <button
                @click="irAReservar"
                class="px-3 sm:px-4 py-2 sm:py-2.5 bg-slate-900 text-white font-medium rounded-xl text-sm hover:scale-[1.02] hover:bg-slate-800 active:scale-95 transition-all duration-200 whitespace-nowrap"
              >
              ...
              <button
                @click="irAMisTurnos"
                class="px-3 sm:px-4 py-2 sm:py-2.5 bg-white border border-slate-300 text-slate-700 font-medium rounded-xl text-sm hover:bg-slate-50 transition-all duration-200 whitespace-nowrap"
              >
                <span class="hidden sm:flex items-center gap-1.5">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
          ...
          <template v-else>
            ...
            <div class="hidden md:flex items-center gap-3">
              <button
                @click="goLogin"
                class="px-4 py-2.5 bg-white border border-slate-300 text-slate-700 font-medium rounded-xl text-sm hover:bg-slate-50 transition-all duration-200"
              >
                Ingresar
              </button>
              <button
                @click="goRegister"
                class="px-4 py-2.5 bg-slate-900 text-white font-medium rounded-xl text-sm hover:scale-[1.02] hover:bg-slate-800 active:scale-95 transition-all duration-200"
              >
                Crear Cuenta
              </button>
            </div>
            ...
            <div class="md:hidden relative" ref="guestMenuRef">
              <button
                @click="toggleGuestMenu"
                class="p-2 rounded-lg hover:bg-slate-100 transition-colors"
                aria-label="Menú"
              >
                <div class="w-6 h-6 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center">
              ...
              <transition
                ...
                leave-to-class="transform opacity-0 scale-95"
              >
                <div v-if="guestMenuOpen" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-50">
                  <div class="px-4 py-3 border-b border-slate-100">
                    <div class="text-sm font-semibold text-slate-900">Bienvenido</div>
                    <div class="text-xs text-slate-500">Accede o regístrate</div>
                  </div>
                  <button
                    @click="goLogin"
                    class="flex items-center gap-2 w-full px-4 py-3 text-sm text-slate-700 hover:bg-slate-50"
                  ...
                  <button
                    @click="goRegister"
                    class="flex items-center gap-2 w-full px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 border-t border-slate-100"
                  >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
// Cerrar menús móviles al hacer clic fuera (autenticado e invitado)
const closeMobileMenusOnOutside = (event: MouseEvent) => {
  const target = event.target as Node | null
  if (!target) return

  const clickedInsideAuthMenu = !!authMenuRef.value?.contains(target)
  const clickedInsideGuestMenu = !!guestMenuRef.value?.contains(target)

  if (!clickedInsideAuthMenu) {
    mobileMenuOpen.value = false
  }

  if (!clickedInsideGuestMenu) {
...
onMounted(() => {
  document.addEventListener('click', closeMobileMenusOnOutside)
})

onUnmounted(() => {
  document.removeEventListener('click', closeMobileMenusOnOutside)
})
